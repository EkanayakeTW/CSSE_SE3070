 ...
 <div class="modal" id="modal-default">
  ...
  <div class="modal-body">
       <h4 class="modal-title">Route Station From Edit</h4>
    ...
    <div class="row">
        <div class="col-sm-6 form-group">
         ...
         <div class="col-sm-6 form-group">
                 <label>Select Route Station:</label>
		<select class="form-control"  id="routeid2" style="width: 100%;">
                    <option class="disabled-result"> Select Route Station</option>
            ...
            <div class="col-sm-6 form-group">
                ...
                <div class="form-group has-feedback">
                    <input type="text" class="form-control" id="name_ed" placeholder="stationId">
                </div>
            </div>       
 ...
 <div class="register-box-body">
    <h3 class="login-box-msg">Set Route Order</h3>
    ...
    <form action="#" method="post">
		...
		<select class="form-control"  id="routeid" style="width: 100%;">
            ...
            <div class="col-sm-4 form-group">
                <label>Route Order: </label>
                <div class="form-group has-feedback">
                    <input type="text" class="form-control" id="name" placeholder="Order Number">
                </div>
            </div>
        ...
        <div class="col-12">
          <div class="card">
            <div class="card-header">
              <h3 class="card-title">Data Table With Route Station Order</h3>
            </div>
            <!-- /.card-header -->
            <div class="card-body">
...
var routeId="",routename="";
              ...
              for(var i=0;i<json.length;i++){
                  id=json[i]['routeId'];
                  name=json[i]['r_name'];
                  routeId=json[i]['stationId'];
                  routename=json[i]['routeOrder'];
                 
                  
trHTML += '<tr><td>' + id+ '</td><td>' + name+ '</td><td>' + routename+ '</td>\n\
<td><button type="button" onclick=edit(\''+encodeURIComponent(id)+'\',\''+encodeURIComponent(name)+'\',\''+encodeURIComponent(routeId) +'\',\''+encodeURIComponent(routename) +'\'); \n\
...
<td><button type="button" onclick=busdetails_delete('+id+'); class="btn btn-default bg-red">Delete</button></tr>';
  ...
  function save_station_route() {
     ...
     $.ajax({
            type: "POST",
            url: "Add_New_route_station_data",
...
function busdetails_updates(){
     ...
     var routeid2 = document.getElementById("routeid2").value;
         ...
         $.ajax({
            type: "POST",
            url: "stationroutes_data_updates",
...
function load_station_type(){
    $.ajax({
        contentType: "application/json",
        type: "POST",
        url: "stationId_id_select",
          success: function (data) {
              var json = JSON.parse(data);
            
              for(var i=0;i<json.length;i++){
                  var IDD  = json[i]['stationId'];
                   var name= json[i]['stationName'];
...
function refresh()
    {
    loadpagecontaint('Route_ststtion_page');
